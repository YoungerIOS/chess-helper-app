...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        logger.debug(
            f"屏幕尺寸={screen_width}x{screen_height}, 窗口尺寸={width}x{height}"
        )
        ...
        main_layout.setContentsMargins(
            self.spacing_base, self.spacing_base, self.spacing_base, self.spacing_base
        )
        ...
        # 连接信号与槽
        self.resume_polling_signal.connect(
            lambda: self.check_platform_change(notify_always=False, schedule_next=True)
        )
        ...
        middle_buttons_layout.setContentsMargins(
            0, 0, 0, 0
        )  # 移除底部边距，保持与主布局间距一致
        ...
        # 设置初始选中状态
        self.continuous_action.setChecked(
            context.analysis_mode == "continuous"
        )  # 根据配置设置初始状态
        self.timer_action.setChecked(
        ...
        self.settings_menu.addSeparator()
        self.jj_training_action = self.settings_menu.addAction("采集JJ训练数据")
        self.jj_training_action.setCheckable(True)
        self.jj_training_action.setChecked(context.jj_training_recording_enabled)
        self.jj_training_action.triggered.connect(self.toggle_jj_training)
        ...
        control_layout.addWidget(
            self.param_btn, 2
        )  # Stretch factor 2 (Wider, same as others)
        ...
        self.movetime_action.triggered.connect(
            lambda: self.on_param_selected("movetime")
        )
        ...
        control_layout.addLayout(
            param_layout, 3
        )  # Stretch factor 3 (Most space for value)
        ...
        self.auto_mover = AutoMoveController(
            context,
            window_provider=lambda: self.platform_detector.find_game_window(
                context.platform
            ),
        ...
        QTimer.singleShot(
            100,
            lambda: self.check_platform_change(notify_always=True, schedule_next=True),
        )
    ...
    def check_platform_change(self, notify_always=False, schedule_next=False):
        ...
        """
        logger.debug(
            f"检查平台变化: is_running={self.is_running}, schedule_next={schedule_next}"
        )
        try:
            # 1. 尝试查找当前平台窗口
            ...
            if result:
                ...
                if detected_platform != context.platform:
                    logger.info(
                        f"自动切换平台: {context.platform} -> {detected_platform}"
                    )
                    ...
                    self.update_text(
                        f"检测到游戏: {platform_display_name}", MessageType.STATUS
                    )
            else:
                # 情况1: 之前检测到了游戏，现在没了 -> 游戏被关闭
                if self.last_detected_platform is not None:
                    self.update_text(
                        "游戏窗口已关闭，等待重新启动...", MessageType.ERROR
                    )
                    self.last_detected_platform = None
            ...
            if schedule_next and not self.is_running:
                QTimer.singleShot(
                    2000,
                    lambda: self.check_platform_change(
                        notify_always=False, schedule_next=True
                    ),
                )
    ...
    def check_queue(self):
        ...
        if (
            ...
            and self.capture_manager is not None
            and self.capture_manager.ready_event.is_set()
            and context.auto_move_enabled
            and context.platform == "JJ"
            and hasattr(self, "auto_mover")
        ):
            self.auto_mover.scan_rematch_button()

        # 一次处理当前所有控制消息。旧实现每100ms只取一条，搜索遥测
        # 会把“轮到我方”和MOVE_CODE堵住，秒走场景下甚至来不及自动应手。
        pending_count = self.ui_message_queue.qsize()
        for _ in range(pending_count):
            try:
                result = self.ui_message_queue.get_nowait()
            except queue.Empty:
                break
            if self._handle_ui_message(result):
                return

        # 引擎遥测不进入控制FIFO，只绘制本周期内最新的一条。
        engine_info = message_bus.take_latest_engine_info()
        if engine_info is not None:
            self._handle_ui_message(engine_info)

    def _handle_ui_message(self, result):
        """处理一条UI消息；返回True表示本周期应立即停止分发。"""
        ...
        if result.type == MessageType.CHANGE:
            self.board_display.update_board(
                result.kwargs["array"], step_info=result.kwargs.get("step_info")
            )
            ...
            self.board_display.update_arrow(
                result.content, result.kwargs["is_red"]
            )
            if (
                result.kwargs.get("auto_execute")
                and context.auto_move_enabled
                and hasattr(self, "auto_mover")
            ):
    ...
    def toggle_auto_move(self, checked=None):
        ...
        enabled = (
            self.auto_move_action.isChecked() if checked is None else bool(checked)
        )
    ...
    def toggle_capture_depth(self, checked=None):
        """切换我方被吃棋子后的动态搜索加深。"""
        enabled = (
            self.capture_depth_action.isChecked() if checked is None else bool(checked)
        )
        ...
        self.refresh_engine_param_label()
        message = (
            "被吃子加深已开启（本局从0重新计数）" if enabled else "被吃子加深已关闭"
        )
        self.update_text(message, MessageType.STATUS)

    def toggle_jj_training(self, checked=None):
        """显式控制训练数据写盘，避免默认采集占用大量空间。"""
        enabled = (
            self.jj_training_action.isChecked() if checked is None else bool(checked)
        )
        self.jj_training_action.setChecked(enabled)
        context.jj_training_recording_enabled = enabled
        if enabled:
            message = "JJ训练数据采集已开启；开始辅助后写入独立会话"
        else:
            message = "JJ训练数据采集已关闭，当前会话正在收尾"
        self.update_text(message, MessageType.STATUS)

    def refresh_engine_param_label(self):
        """显示当前参数；固定深度模式同时显示本局吃子加成。"""
        if not hasattr(self, "param_label") or not hasattr(
            self, "selected_engine_param"
        ):
            return
        engine_params = context.get_engine_params()
        key = self.selected_engine_param
    ...
    def event(self, event):
        """处理自定义事件"""
        if event.type() == CONFIRM_POSITION_EVENT and hasattr(
            self, "cursor_pos_clicked"
        ):
            # 从保存的属性获取光标位置
            ...
            def refresh_worker():
                try:
                    ...
                    if success:
                        ...
                        message_bus.publish_error(
                            "截图服务繁忙，本次刷新未完成，请稍后重试"
                        )
                finally:
                    self._manual_refresh_pending = False
    ...
    def on_change_move(self):
        ...
        try:
            ...
            threading.Thread(
                target=process.request_alternative_move, daemon=True
            ).start()
    ...
    def show_param_menu(self):
        """显示参数菜单"""
        self.param_menu.exec_(
            self.param_btn.mapToGlobal(self.param_btn.rect().bottomLeft())
        )
    ...
    def update_engine_analysis(self, info):
        ...
        if score_type == "mate" and score is not None:
            evaluation = (
                f"我方将杀{abs(score)}步" if score > 0 else f"对方将杀{abs(score)}步"
            )
    ...
    def show_board_menu(self):
        """显示棋盘定位菜单"""
        self.board_menu.exec_(
            self.board_btn.mapToGlobal(self.board_btn.rect().bottomLeft())
        )
    ...
    def update_icons(self):
        ...
        icon_name = (
            "pulldown_arrow_black.png"
            if context.theme == "light"
            else "pulldown_arrow.png"
        )
    ...
    def show_game_menu(self):
        """显示游戏选择菜单"""
        self.game_menu.exec_(
            self.game_btn.mapToGlobal(self.game_btn.rect().bottomLeft())
        )
...
class ManualPositioner(QObject):
    ...
    def start_positioning(self):
        ...
        self.main_window.text_display.setText(
            '<span style="color: red;">第1步：点击棋盘左上角顶点<br>右键点击可取消定位</span>'
        )
    ...
    def cancel_positioning(self):
        ...
        self.main_window.text_display.setText(
            '<span style="color: blue;">定位已取消</span>'
        )
        logger.info("手动定位已取消")
    ...
    def create_position_dot(self, is_temp=False, x=None, y=None):
        ...
        if is_temp:
            ...
            try:
                ...
                if (
                    not board_region
                    or "x" not in board_region
                    or "y" not in board_region
                ):
    ...
    def is_temp_position_dot(self):
        ...
        return (
            self.position_dot
            and hasattr(self.position_dot, "property")
            and self.position_dot.property("is_temp") == True
        )
    ...
    def on_mouse_click(self, x, y, button, pressed):
        ...
        if button == mouse.Button.left:
            ...
            QApplication.instance().postEvent(
                self.main_window, QEvent(CONFIRM_POSITION_EVENT)
            )
            ...
            QApplication.instance().postEvent(
                self.main_window, QEvent(CANCEL_POSITION_EVENT)
            )
            return False  # 停止监听
        return True
    ...
    def confirm_position(self, cursor_pos):
        ...
        if self.positioning_step == 1:
            ...
            self.main_window.text_display.setText(
                '<span style="color: orange;">第2步：点击棋盘右下角顶点<br>右键点击可取消定位</span>'
            )
            ...
            if success:
                ...
                self.main_window.text_display.setText(
                    '<span style="color: green;">2角定位完成!</span>'
                )
            else:
                # 定位失败，显示错误信息
                self.main_window.text_display.setText(
                    '<span style="color: red;">2角定位失败，请重试</span>'
                )
                # 重置定位状态，允许重新开始
                self.positioning_step = 0
                self.positioning_coords = []
    ...
    def process_two_corner_positioning(self):
        ...
        board_region = {"x": x, "y": y, "width": width, "height": height}
